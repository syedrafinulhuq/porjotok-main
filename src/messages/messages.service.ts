...
export class MessagesService {
  ...
  async leaveConversation(conversationId: string, userId: string): Promise<void> {
    ...
    // If it's a group chat and there are still participants, save the updated participant list
    if (conversation.isGroupChat && conversation.participants.length > 0) {
      await this.conversationRepository.save(conversation);
    } else if (conversation.participants.length <= 1) {
      // If there are no or only one participant left, remove the conversation
      await this.conversationRepository.remove(conversation);
    } else {
      // For a 1-1 chat, remove the conversation if one person leaves
      await this.conversationRepository.remove(conversation);
    }
  }
}